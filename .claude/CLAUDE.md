...
- Sitio estático (HTML, CSS y JS puro: `index.html`, `style.css`, `script.js`) con scroll cinematográfico y video del hero que avanza con el scroll. Detalles en `README.txt`.
- Para probar local, `.claude/serve.js` (soporta Range requests, necesarios para el video). Rama principal: `master`.
- Si cambias `videos/hero.mp4`, ajusta el alto de la sección `.scrub` en `style.css` según la duración.
- La identidad de la marca manda sobre la lista de prohibidos de diseño.
